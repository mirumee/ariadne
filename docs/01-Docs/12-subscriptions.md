...
> Because of their nature, subscriptions require an asynchronous ASGI server. Ariadne currently supports subscriptions over WebSockets and Server-Sent Events (SSE).
> If you are using WebSockets with `uvicorn` you need to `pip install websockets` otherwise you'll get `Could not connect to websocket endpoint ws://localhost:8000/. Please check if the endpoint url is correct.`
>
> *WSGI*-based servers (including Django) are synchronous in nature and *unable* to handle long-lived WebSocket or SSE connections, which makes them incapable of implementing subscriptions directly.
>
...
In the world of GraphQL clients, there are two WebSocket-based subscription protocols that clients can implement for subscribing to a GraphQL server.


### `subscriptions-transport-ws`

Default protocol used by Ariadne. The client library for it is still widely used although it's no longer maintained. It has the benefit of being supported by GraphQL Playground out of the box.
...
New protocol that replaced `subscriptions-transport-ws`. It's actively maintained and supported by Apollo Studio Explorer.
...
- `generator` is a function that yields data we're going to send to the client. It can be either an asynchronous generator or a synchronous generator.
...
> Make sure you understand how Python generators (both asynchronous and synchronous) work before attempting to use subscriptions.
...
Publisher-consumer ("pub-sub") is a pattern in which parts of the system listen for ("subscribe to") events ("messages") from other parts of the system, usually reacting to them with very small delay.
...
Where should publishing code live? Simplest answer is _at the same place that events occur that you would like your users to subscribe to_. Here are a few examples:

- In GraphQL mutations: `postComment` mutation could publish an event to notify other clients on the same page that a new comment was posted.
- In task queues: `process_video_file` Celery task could publish an event with current progress on processing an uploaded video file.
- In regular views: your JSON API or standard HTTP form view can send an event that a contact form was sent to notify customer service members online.


### Subscriber

Unlike publishers, which can go anywhere, subscribers in GraphQL APIs have a single dedicated place: subscription _sources_:
...
Please note that because the `params` value is set by the client there are no guarantees on what type and contents this value will be. Due care needs to be taken here:
...
If your client has two separate UI components (eg. notification bell on the navbar and list of online users), and those components do GraphQL `subscribe` queries, `context_value` will be run for each of those separately while `on_connect` will only be run once.
...
> **Warning:** This feature is considered experimental. It was implemented for feature parity with an older version of Apollo Server. Its final shape (or presence in future Ariadne releases) is not decided yet. Generally you should try using `on_connect` and `on_disconnect` first before using these features.
...
def counter_resolver(count: int, info: Any) -> int:
    return count
