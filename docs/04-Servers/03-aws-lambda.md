...
Ariadne Lambda is an extension to Ariadne itself to enable running [ASGI](./asgi) applications on AWS Lambda:
...
> **Note:** If you need your Lambda function to offer other API endpoints in addition to GraphQL, you can combine your Ariadne app with [Starlette](../Integrations/starlette-integration) or [FastAPI](../Integrations/fastapi-integration) along with [Lynara](https://github.com/mirumee/lynara), which wraps the app to handle HTTP requests from AWS.

## Minimal Lambda Handler Example

If you want to skip the HTTP stack altogether, you can execute the queries directly using the [`graphql_sync`](../API-reference/api-reference#graphql_sync):
