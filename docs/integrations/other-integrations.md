...
1. Use [`make_executable_schema`](../api-reference/api-reference.md#make_executable_schema) to create executable schema instance.
...
5. Call [`graphql_sync`](../api-reference/api-reference.md#graphql_sync) with schema, parsed JSON and any other options that are fit for your implementation.
6. [`graphql_sync`](../api-reference/api-reference.md#graphql_sync) returns tuple that has two values: `boolean` and `dict`. Use dict as data for JSON response, and boolean for status code. If boolean is `true`, set response's status code to `200`, otherwise it should be `400`

See the [Flask integration](flask-integration.md) for implementation of this algorithm using Flask framework.

## Asynchronous servers

If your server stack supports ASGI, you can use [`graphql`](../api-reference/api-reference.md#graphql) to execute GraphQL queries asynchronously and [`subscribe`](../api-reference/api-reference.md#subscribe) for websocket connections initialized by subscriptions.
...
3. Call [`combine_multipart_data`](../api-reference/api-reference.md#combine_multipart_data) with `operations`, `map` and data structure from step 2 as its values.
4. Call [`graphql`](../api-reference/api-reference.md#graphql) or [`graphql_sync`](../api-reference/api-reference.md#graphql_sync) with value returned by `combine_multipart_data`.
